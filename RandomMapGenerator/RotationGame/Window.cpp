...
bool Window::OnCreate(){
	...
	window = SDL_CreateWindow("Rotation Mania", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, width, height, SDL_WINDOW_SHOWN);		//changed the window name to a temp game name.
	if (window == nullptr) {
		std::cout << "SDL_Error: " << SDL_GetError() << std::endl;
		return false;
	}
